...
def main(
    ...
    small_teacher_results = set_index(
        pd.read_csv(f"{output_dir}/small_teacher.csv")
    ).rename(columns={"value": "small_teacher"})
        ...
        for e in fs.ls(output_dir):
            ...
            df = df.join(
                set_index(pd.read_csv(f"gs://{e}")).rename(
                    columns={
                        "value": f"val_{e.split('.csv')[0].split('_ckpt')[0].split('_val_loss_total_')[1]}"
                    }
                )
            )
    else:
    ...
    df = df.join(small_teacher_results)
    df = df.join(teacher_results)

    df.to_markdown("/tmp/p.md")
    print(df)
